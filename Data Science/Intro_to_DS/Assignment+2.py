...
def answer_two():
    ...
    return df.sort("Difference")["Difference"].index[-1]
answer_two()
...
def answer_three():
    f = df[(df["Gold"] > 0) & df["Gold.1"] > 0]
    f["Difference"] = ((f["Gold"]-f["Gold.1"])/f["Gold.2"])
    return f.sort("Difference",ascending=False)["Difference"].index[0]
answer_three()
...
def answer_five():
    a = pd.DataFrame(index=census_df["STATE"].unique(),columns=["COUNT"])
    [a.set_value(i,"COUNT",len(census_df[census_df["STATE"] == i]))  for i in a.index]
    return census_df[census_df["STATE"] == a.sort("COUNT")[-1:].index[0]]["STNAME"].unique()[0]
answer_five()
...
def answer_six():
    col = 'POPESTIMATE2015' 
    f = pd.DataFrame(cdf[cdf['SUMLEV'] == 50]).groupby(['STNAME'])[col].nlargest(3)
    f = f.reset_index()
    return f.groupby(['STNAME']).sum()[col].nlargest(3).index.tolist()
answer_six()
...
def answer_eight():
    a = census_df[(census_df['REGION'] <= 2)]
    b = a[a['CTYNAME'].str.contains("Washington")]
