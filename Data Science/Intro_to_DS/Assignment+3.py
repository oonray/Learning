...
def answer_three():
    Top15 = answer_one()
    avgGDP = Top15[[str(i) for i in range(2006,2016)]].transpose().mean().sort_values(ascending=False)
    return avgGDP
answer_three()
...
def answer_four():
    T= answer_three().index[5]
    t = answer_one()[[str(i) for i in range(2006,2016)]].transpose()
    return t[T].max()-t[T].min()
answer_four()
...
def answer_five():
    Top15 = answer_one()
    return Top15["Energy Supply per Capita"].dropna().mean()
answer_five()
...
def answer_six():
    Top15 = answer_one()
    a = Top15.sort("% Renewable",ascending=False).iloc[0]
...
def answer_seven():
    Top15 = answer_one()
    Top15["RatioCit"] = Top15["Self-citations"]/Top15["Citations"]
    return Top15["RatioCit"].idxmax(),Top15["RatioCit"].max()
answer_seven()
...
def answer_eight():
    ...
    return  Top15.sort('PopEst',ascending=False)['PopEst'].index[2]
answer_eight()
...
def answer_ten():
    ...
    HighRenew =Top15.sort("Rank",ascending = True)
...
def answer_eleven():
    ...
    a = (Top15.reset_index().replace(ContinentDict ))
    c={'size':a["index"].value_counts(), 
       'sum': [a[a['index']==i]["PopEst"].sum() for i in a["index"].unique()],
       'mean':[a[a['index']==i]["PopEst"].mean() for i in a["index"].unique()],
       'std':[a[a['index']==i]["PopEst"].std() for i in a["index"].unique()]}
    b = pd.DataFrame(index=a["index"].unique(),data=c)
    return b
answer_eleven()
...
def answer_twelve():
    ...
    Top15["CutRenew"] = pd.cut(Top15["% Renewable"],5)
    return pd.MultiIndex([Top15["Continent"],Top15.index],)#Top15.groupby(["Continent","CutRenew"])
answer_twelve()
